...
export const getAtaAccount = async (
  mint: anchor.web3.PublicKey,
  wallet: anchor.web3.PublicKey
): Promise<anchor.web3.PublicKey> => {
  let userAssociatedTokenAccount = await spl.getAssociatedTokenAddress(mint, wallet);
...
export const createAtaAccount = async (
  ...
  const userAssociatedTokenAccount = await spl.getAssociatedTokenAddress(mint, user);

  const txFundTokenAccount = new anchor.web3.Transaction();
  txFundTokenAccount.add(
    spl.createAssociatedTokenAccountInstruction(fee_payer.publicKey, userAssociatedTokenAccount, user, mint)
  );
...
export const transferSol = async (
  ...
  await provider.sendAndConfirm(txTransfer);
};
...
export const createUserAndAssociatedWallet = async (
  ...
  if (mint) {
    ...
    if (!amount && createAta == false) {
      ...
      if (createAta) {
        txFundTokenAccount.add(
          spl.createAssociatedTokenAccountInstruction(user.publicKey, userAssociatedTokenAccount, user.publicKey, mint)
        );
      }

      if (amount) {
        txFundTokenAccount.add(
          spl.createMintToInstruction(mint, userAssociatedTokenAccount, provider.wallet.publicKey, amount)
        );
      }
...
export const createAssociatedWalletAndMint = async (
  provider: anchor.AnchorProvider,
  ...
  const userAssociatedTokenAccount = await spl.getAssociatedTokenAddress(mint, wallet.publicKey);
...
export const transferToken = async (
  ...
  txTransfer.add(spl.createTransferInstruction(source, destination, owner.publicKey, amount));
...
export const approveToken = async (
  ...
  txTransfer.add(spl.createApproveInstruction(account, delegate, owner.publicKey, amount));
  await provider.sendAndConfirm(txTransfer, [owner]);
};
